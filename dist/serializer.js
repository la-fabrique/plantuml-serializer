import { C4Person, C4System, C4Container, C4Component, C4Relationship, C4Boundary, C4_WORKSPACE_LAYOUT_TOP_DOWN, } from './types';
function serializeElement(element, indent = 0) {
    ...
    if (element instanceof C4Boundary) {
        ...
        lines.push(indentString(`${elementName}(${alias}${alias1}${alias2}${label}${technology}${description}${link})`, indent));
    }
    return lines;
}
function indentString(txt, count = 1) {
    const regex = /^/gm;
    return txt.replace(regex, '    '.repeat(count));
}
export default {
    serialize: (workspace, options = {
        eol: '\n',
    }) => {
        let lines = ['@startuml'];
